...
[Return to home page](https://lichess4545stats.github.io/docs/)
...
if(scrub_cheats){
  ...
  tos_violators <- character(0)  # Empty vector if not scrubbing
}
```
...
season_reports_url <- "https://lichess4545stats.github.io/docs/season_stats.html"
...
# homepage_url <- "https://lichess4545stats.github.io/docs/season_stats.html"
#
...
# Scrub ToS violators
# tos_violators variable was already loaded in the setup chunk from identify_tos_violators.R
# Create toscrub pattern (will be used later too)
if(length(tos_violators) > 0) {
  toscrub <- str_c(tos_violators, collapse = "|")
  awards$Winner <- stringr::str_remove_all(awards$Winner, toscrub)
  # Tidy up any after-effects
  awards$Winner <- str_replace_all(awards$Winner, "^,\\s", "")
  awards$Winner <- str_replace_all(awards$Winner, ",\\s,", ",")
  awards$Winner <- str_replace_all(awards$Winner, ",\\s,", ",")
} else {
  toscrub <- ""  # Empty string for later checks
}
